...
def index():
	...
	if form.validate_on_submit():
		...
		dataset, links, query_list, linked_epic_query_string, error, query_epic_set = get_jira_query_results(form.query.data)
		...
		if error is not None:
			...
			if linked_epic_query_string != 'issuekey in ()':    
				...
				linked_epic_dataset, _, _, _, _, _ = get_jira_query_results(linked_epic_query_string)
			...
			if query_epic_set is not None:
				for epic_key in query_epic_set:
					epic_link_issues = []
					epic_link_dataset, _, _, _, _, _ = get_jira_query_results('"Epic Link" = "{}"'.format(epic_key))
